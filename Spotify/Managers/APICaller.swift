        ...
        createRequest(with: URL(string: Constants.baseAPIURL + "/search?limit=10&type=album,artist,playlist,track&q=\(query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")"), type: .GET) { request in
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    ...
                    searchResults.append(contentsOf: result.tracks.items.compactMap { tracks in .track(model: tracks) })
                    searchResults.append(contentsOf: result.albums.items.compactMap { album in .album(model: album) })
                    searchResults.append(contentsOf: result.artists.items.compactMap { artist in .artist(model: artist) })
                    searchResults.append(contentsOf: result.playlists.items.compactMap { playlist in .playlist(model: playlist) })
